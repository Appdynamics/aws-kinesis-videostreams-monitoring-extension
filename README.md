...
## Use Case
Captures statistics for Amazon Kinesis Video Streams from Amazon CloudWatch and displays them in the AppDynamics Metric Browser.
...
2. Copy and unzip `AWSKinesisVideoStreamsMonitor-<version>.zip` from `target` directory into `<machine_agent_dir>/monitors/`
3. Edit `config.yml` file in `AWSKinesisVideoStreamsMonitor` and provide the required configuration (see Configuration section).
...
1. If SIM is enabled, then use the following metricPrefix - 
   ...
   `metricPrefix: "Server|Component:<COMPONENT_ID>|Custom Metrics|AWS Kinesis Video Streams|"`.
...
3. If you want to encrypt the **awsAccessKey** and **awsSecretKey** then follow the "Credentials Encryption" section and provide the encrypted values in **awsAccessKey** and **awsSecretKey**. Configure `enableDecryption` of `credentialsDecryptionConfig` to `true` and provide the encryption key in `encryptionKey`.
   ...
   ```
4. To report metrics only from specific dimension values, configure the `dimesion` section. Dimension for Kinesis Video Streams is `StreamName`. For example to report metrics only from `StreamName` dimension with value `Sample`, configure `dimensions` as below -
...
## Metrics
Typical metric path: `Application Infrastructure Performance|<Tier>|Custom Metrics|AWS Kinesis Video Streams|<Account Name>|<Region>|StreamName|<stream_name>|` followed by the metrics defined in the link below:

- [Kinesis Video Streams Metrics](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/akac-metricscollected.html)
...
Always feel free to fork and contribute any changes directly here on [GitHub](https://github.com/Appdynamics/aws-kinesis-videostreams-monitoring-extension).
